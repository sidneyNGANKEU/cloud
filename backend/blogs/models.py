...
def upload_to_path(instance: 'Blog', filename: str) -> str:
    ...
    if filename.lower().endswith(('.mp4', '.avi', '.mov', '.mkv', '.wmv')):
        return f'{settings.BLOG_VIDEO_DIR_NAME}/{user_id}-{blog_id}-{filename}'
    elif filename.lower().endswith(('.jpg', '.jpeg', '.png', '.gif','jfif')):
        return f'{settings.BLOG_IMAGE_DIR_NAME}/{user_id}-{blog_id}-{filename}'
    else:
        # Gérer d'autres types de fichiers selon les besoins
        return f'{settings.BLOG_OTHER_DIR_NAME}/{user_id}-{blog_id}-{filename}'
...
class Blog(models.Model):
    ...
    class Category(models.TextChoices):
        ESPASE_CLIENT_CLOUD = 'espase client cloud', 'ESPASE CLIENT CLOUD'
        VCLOUD_DIRECTOR = 'vcloud director', 'VCLOUD DIRECTOR'
        MACHINE_VIRTUELLE = 'machine virtuelle', 'MACHINE VIRTUELLE'
    ...
    cover_image = models.FileField(upload_to=upload_to_path, max_length=200, blank=True, null=True)

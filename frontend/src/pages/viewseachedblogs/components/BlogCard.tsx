...
export const BlogCard = ({ blogData }: any): JSX.Element => {
  ...
  const isVideo = (url: string): boolean => {
    // Obtenir l'extension de fichier à partir de l'URL
    const extension = url.split('.').pop()?.toLowerCase() || '';
    
    // Liste des extensions vidéo valides
    const videoExtensions = ['mp4', 'avi'];
    
    // Vérifier si l'extension fait partie des extensions vidéo valides
  ...
  return (
    <div className="w-[95%] sm:w-full mt-4 flex justify-between items-center p-4 bg-white hover:scale-105 duration-300 rounded-md border-[1.5px] border-slate-200 shadow-md shadow-slate-300">
      {/** Cover image */}
      {isVideo(blogData.cover_image) ? (
        // Si le média est une vidéo, afficher la balise vidéo
        <video controls className="w-[30%] h-[11.25rem] rounded-md object-cover">
          <source src={blogData.cover_image} type="video/mp4"/>
          Your browser does not support the video tag.
        </video>
      ) : (
        // Si le média n'est pas une vidéo, afficher la balise img

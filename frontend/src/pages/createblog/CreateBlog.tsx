...
export const CreateBlog = (): JSX.Element => {
  ...
  const coverImageOnChange = (e: React.ChangeEvent<HTMLInputElement>): void => {
    ...
    if (files) {
      ...
      // Vérifier si le fichier est une vidéo (par exemple, avec une extension .mp4)
      const isVideo = selectedFile.type.startsWith('video/');
      
      if (isVideo) {
        // Vous pouvez afficher un aperçu de la vidéo si nécessaire
        const videoReader = new FileReader();
        videoReader.onloadend = () => {
          setPreviewCoverImage(videoReader.result as string);
        };
        videoReader.readAsDataURL(selectedFile);
      } else {
        // Si c'est une image, afficher un aperçu d'image comme avant
        const imageReader = new FileReader();
        imageReader.onloadend = () => {
          setPreviewCoverImage(imageReader.result as string);
        };

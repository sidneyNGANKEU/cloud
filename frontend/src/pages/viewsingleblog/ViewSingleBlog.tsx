  ...
  return (
    <div className="mx-auto max-w-[1080px] mt-20 w-full">
      {isLoadingFetchBlog ? (
        ...
        <>
          {/** Cover image */}
           <div className="w-full sm:h-[25rem] p-5">
            {blogData?.cover_image && (
              <>
                {(blogData.cover_image.toLowerCase().endsWith('.mp4') || 
                  blogData.cover_image.toLowerCase().endsWith('.avi') || 
                  blogData.cover_image.toLowerCase().endsWith('.mov') || 
                  blogData.cover_image.toLowerCase().endsWith('.mkv') || 
                  blogData.cover_image.toLowerCase().endsWith('.wmv')) ? (
                  <video controls className="w-full h-[10rem] sm:h-full rounded-md object-cover">
                    <source src={blogData.cover_image} type="video/mp4" />
